...
module MLRecommender = struct
  ...
  (** Abstraction Function: [embeddings] represents the learned latent factors
      for users and videos in the matrix factorization model.
      ...
      - [learning_rate] > 0.0.
      - [lambda] >= 0.0.
      - All arrays in [user_embeddings] and [video_embeddings] have length
        [embedding_dim]. *)
...
module CFRecommender = struct
  ...
  (** [get_all_users ?users_dir ()] returns a list of all users by reading the
      ...
      history and stats. Requires: The users directory exists (defaults to
      "data/users"). Returns: A list of users with their complete data loaded.
  *)
  let get_all_users ?(users_dir = "data/users") () : user list =
    try
      ...
      |> List.map (fun user_name ->
          ...
          (* Load genre counts from stats.csv using Csv.load for robust
             parsing *)
                  ...
                  (fun row ->
                    ...
                    | title :: genre :: liked_str :: watchtime_str :: _ ->
                        let video = { title; ascii = ""; genre } in
                        {
                          video;
                          watchtime =
                            (try float_of_string watchtime_str with _ -> 0.0);
                          liked =
                            (try bool_of_string liked_str with _ -> false);
                        }
                    ...
                    | _ ->
                        let video =
                          { title = "Unknown"; ascii = ""; genre = "Unknown" }
                        in
          ...
          { name = user_name; vid_history; genre_counts })
    with Sys_error _ -> []
  ...
  let embed_user_list user_list =
    ...
    List.iter
      (fun (user : user) -> MLRecommender.train_on_history embeddings user)
      user_list;
  ...
  let%test "test_user_closeness_list_basic" = (
    ...
    | [ (u1, score1); (u2, score2) ] ->
        let check_a =
          u1.name = "User A" && abs_float (score1 -. 1.0) < 0.0001
        in
  ...
  let%test "test_get_top_k" = (
    ...
    let input_list = [ (user_low, 0.1); (user_high, 0.9); (user_mid, 0.5) ] in
    let result = get_top_k_users input_list 2 in
    match result with
    | [ (u1, s1); (u2, s2) ] ->
        let check_1 =
          u1.name = "High Match" && abs_float (s1 -. 0.9) < 0.0001
        in
  ...
  let filter_out_seen (candidates : interaction list)
    ...
    List.filter
      (fun cand ->
        not
          (List.exists
             (fun seen -> seen.video.title = cand.video.title)
             seen_history))
      candidates
  ...
  let get_all_videos_from_users (similar_users : (user * float) list)
      (current_user_history : interaction list) =
    let video_table = Hashtbl.create 50 in
    List.iter
      (fun (other_user, closeness_score) ->
        ...
        List.iter
          (fun interaction ->
            ...
            let current_list =
              try Hashtbl.find video_table video_key with Not_found -> []
            in
            Hashtbl.replace video_table video_key
              ((closeness_score, interaction) :: current_list))
          unseen_interactions)
      similar_users;
    video_table
  ...
  let%test "test_aggregate_videos" = (
    ...
    let user_b =
      {
        name = "User B";
        vid_history = [ inter_b_x; inter_b_y ];
        genre_counts = Hashtbl.create 1;
      }
    in
    ...
    let has_score_9 =
      List.exists (fun (s, _) -> abs_float (s -. 0.9) < 0.0001) list_x
    in
    let has_score_5 =
  ...
  let%test "test_score_videos_basic" = (
    ...
    in
    let vid_low =
      { title = "Low Score Video"; ascii = ""; genre = "Genre B" }
    in
  ...
  let get_cf_scores_for_target (target_user : user)
    ...
    match
      List.find_opt (fun (u, _) -> u.name = target_user.name) system_embeddings
    with
    | None -> []
    | Some (_, target_emb) ->
        (* 2. Calculate Closeness *)
        let closeness =
          user_closeness_list target_user.name target_emb system_embeddings
        in
        (* 3. Filter Positive Only (Safety check against enemies!) *)
        let positive_closeness =
          List.filter (fun (_, s) -> s > 0.0) closeness
        in
        (* 4. Get Top Neighbors *)
        let neighbors = get_top_k_users positive_closeness k in
        (* 5. Aggregate Videos *)
        let video_table =
          get_all_videos_from_users neighbors target_user.vid_history
        in
        (* 6. Score Videos *)
...
module HybridRecommender = struct
  ...
  let recommend_hybrid (user : user) (videos : video list)
      (system_embeddings : (user * float array) list) : video option =
    ...
    if unwatched = [] then None
      ...
      (* Cold Start: Just pick a random video if we know nothing about the
         user *)
      Some (List.nth unwatched (Random.int (List.length unwatched)))
    else begin
      let cf_results =
      ...
      (* Convert that list into a Hash Map so we can look it up instantly
         (O(1)) *)
      let cf_score_map = Hashtbl.create 10 in
      List.iter
        (fun (v, s) -> Hashtbl.replace cf_score_map v.title s)
        cf_results;
      ...
      let scored =
        List.map
          (fun v ->
            ...
            let cf_s =
              try Hashtbl.find cf_score_map v.title with Not_found -> 0.0
            in
