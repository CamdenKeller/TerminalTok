...
module MLRecommender = struct
  ...
  type embeddings = {
    user_embeddings : (string, float array) Hashtbl.t;  (* user_name -> embedding *)
    video_embeddings : (string, float array) Hashtbl.t; (* video_title -> embedding *)
    learning_rate : float;
  ...
  let get_user_embedding (embeddings : embeddings) (user_name : string) : float array =
    try
      Hashtbl.find embeddings.user_embeddings user_name
    with Not_found ->
      let embedding = Array.init embedding_dim (fun _ -> Random.float 0.1 -. 0.05) in
      Hashtbl.add embeddings.user_embeddings user_name embedding;
      embedding
  
  (* Get or create embedding for video *)
  let get_video_embedding (embeddings : embeddings) (video_title : string) : float array =
    try
      Hashtbl.find embeddings.video_embeddings video_title
    with Not_found ->
      let embedding = Array.init embedding_dim (fun _ -> Random.float 0.1 -. 0.05) in
  ...
  let interaction_to_score (interaction : interaction) : float =
    ...
    (* Watch time weight: 40% 
       Normalize by sigmoid function so longer watch = higher score
       Formula: 1 - exp(-watchtime/5) gives 0 to 1 curve #Lit math*)
    let watch_score = 
  ...
  let train_step (embeddings : embeddings) 
                 (user_name : string) 
    ...
    for i = 0 to embedding_dim - 1 do
      let user_grad = error *. video_emb.(i) -. embeddings.lambda *. user_emb.(i) in
      let video_grad = error *. user_emb.(i) -. embeddings.lambda *. video_emb.(i) in
  ...
  let predict_score (embeddings : embeddings) 
                    (user_name : string) 
    ...
    1.0 /. (1.0 +. exp (-.score)) (* sigmoid function for bound from 0 to 1*)
  ...
  let recommend_ml (embeddings : embeddings) (user : user) (videos : video list) : video option =
    let unwatched =
    ...
    if unwatched = [] then None
      ...
      let scored_videos = List.map (fun v ->
        let score = predict_score embeddings user.name v.title in
        (v, score)
      ) unwatched in
      
      (* Sort by score descending *)
      let sorted = List.sort (fun (_, s1) (_, s2) -> compare s2 s1) scored_videos in
...
module HybridRecommender = struct
  ...
  let content_based_score (user : user) (video : video) : float =
    try
      let genre_count = float_of_int (Hashtbl.find user.genre_counts video.genre) in
