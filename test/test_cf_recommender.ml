...
let test_recommend_cf _ =
  with_scenario_users (fun base_dir ->
      let shared_video =
        { Types.title = "Shared Video"; ascii = ""; genre = "action" }
      in
      let target_interaction =
        { Types.video = shared_video; watchtime = 20.0; liked = true }
      in
      let target_user =
        {
      ...
      in
      let result =
        Recommender.CFRecommender.recommend_cf ~users_dir:base_dir target_user 5
      in
      match result with
      | Some video ->
          assert_equal ~printer:(fun s -> s) "Hidden Gem" video.title
      | None ->
          assert_failure
            "CF failed to recommend a video despite high similarity!")
