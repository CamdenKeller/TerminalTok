...
let pp_camel (c : video) =
  Printf.sprintf "{title=%S; genre=%S; ascii=%S}" c.title c.genre c.ascii

let pp_video_tuple (t, g, f) =
  Printf.sprintf "(title=%S; genre=%S; file=%S)" t g f
...
let test_parse_valid_json _ =
  ...
  assert_equal ~printer:pp_string "Camel 1" v1.title;
  assert_equal ~printer:pp_string "comedy" v1.genre;
  assert_equal ~printer:pp_string "  O  \n /|\\ " v1.ascii
...
let test_parse_videos _ =
  ...
  assert_equal ~printer:pp_string "Video 1" t1;
  assert_equal ~printer:pp_string "Genre 1" g1;
  assert_equal ~printer:pp_string "path/to/video1.mp4" f1
...
let test_parse_malformed_json _ =
  ...
  assert_raises
    (Yojson.Json_error "Line 1, bytes 15-16:\nUnexpected end of input")
    (fun () -> parse_camels filename);
  Sys.remove filename
