...
let test_recommend_ml_cold_start _ =
  ...
  | Some v -> assert_bool "return one of the available videos" (v = v1 || v = v2)
...
let test_recommend_ml_one_interaction _ =
  ...
  | Some v ->
      assert_bool "Should return one of the unwatched videos" (v = v2 || v = v3)
...
let test_recommend_ml_with_history _ =
  ...
  | Some v -> assert_bool "return an unwatched video" (v = v3 || v = v4)
...
let test_recommend_ml_many_interactions _ =
  ...
  let watched =
    List.init 10 (fun i -> make_video (Printf.sprintf "Watched%d" i) "action")
  in
  ...
  | Some v -> assert_equal new_video v ~msg:"recommend the only unwatched video"
...
let test_recommend_ml_single_unwatched _ =
  ...
  | Some v -> assert_equal v2 v ~msg:"recommend the only unwatched video"
...
let test_recommend_ml_exact_two_interactions _ =
  ...
  | Some v -> assert_equal v3 v ~msg:"recommend using ML model"
...
let suite =
  ...
  >::: [
         ...
         "recommend_ml_many_interactions"
         >:: test_recommend_ml_many_interactions;
         "recommend_ml_single_unwatched" >:: test_recommend_ml_single_unwatched;
         "recommend_ml_exact_two_interactions"
         >:: test_recommend_ml_exact_two_interactions;
       ]
